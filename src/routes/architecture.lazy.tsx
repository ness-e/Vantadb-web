...
const SPECS = [
  { label: "Quantization Methods", val: "SQ8 \u00B7 TurboQuant \u00B7 RaBitQ", desc: "8-bit scalar, 3-bit turbo, or 1-bit RaBitQ \u2014 4\u00D7 to 32\u00D7 memory reduction over full f32 vectors.", amber: false },
  { label: "Graph Traversal", val: "BFS \u00B7 DFS \u00B7 Topo \u00B7 DAG", desc: "Built-in BFS/DFS traversal, topological sort, and DAG cycle detection over directed adjacency edges.", amber: false },
  { label: "Key Size Limit", val: "1 KB", desc: "Identifiers must be compact to ensure lightning-fast pointer indexing in standard LSM memory bounds.", amber: false },
  { label: "Vector Dimensions", val: "32,768 dims", desc: "Configurable HNSW index structures, optimized up to high-end sparse embeddings.", amber: true },
  { label: "Text Content Limit", val: "10 MB per put", desc: "Larger chunk payloads are offloaded directly to binary buffers, preserving planner heap latency floors.", amber: false },
  { label: "Metadata Size", val: "64 KB", desc: "Structured dictionaries for lexical pre-filtering operations before RRF fusion runs.", amber: false },
  { label: "Concurrency Model", val: "1 Writer \u00B7 \u221E Readers", desc: "Safe thread concurrent read executions, write paths synchronize locks to protect journal logs.", amber: false },
  { label: "sync_mode", val: "always | periodic | never", desc: "Set flush WAL constraints depending on durability trade-offs. Periodic sync defaults to 500ms intervals.", amber: false },
  { label: "Engine States", val: "Init \u2192 Ready \u2192 Flush \u2192 Closed", desc: "Exposes state transitions cleanly through FFI to allow hot rebuilding without data losses.", amber: false },
  { label: "Memory Prefetching", val: "madvise / PrefetchVirtualMemory", desc: "Predictive kernel prefetching on Unix (madvise) and Windows (PrefetchVirtualMemory) for cold vector pages.", amber: true },
  { label: "Data Expiry", val: "TTL auto-eviction", desc: "Time-to-live based automatic record expiry with background compaction.", amber: false },
  { label: "Batch Operations", val: "Rayon parallelism", desc: "Batch put/get/delete operations parallelized across all available CPU cores.", amber: false },
];

const SEGMENTS = [
  { id: "ffi", label: "FFI Boundary Translation", share: 12, desc: "Fast PyO3 compilation translating Python dictionaries to native Rust structs with zero memory copies.", bg: "var(--steel)" },
  { id: "planner", label: "Query Optimization", share: 24, desc: "Sifts through filters, sets up lexical scanning ranges, and prepares candidate HNSW entry points.", bg: "var(--muted)" },
  { id: "index", label: "HNSW & BM25 Traversal", share: 48, desc: "Core path searching approximate nearest neighbors and evaluating index statistics in parallel.", bg: "var(--amber)" },
  { id: "wal", label: "WAL Commit & fsync", share: 16, desc: "Forces durability logging, computes transaction CRC32C, and syncs sectors on disks.", bg: "var(--foreground)" },
];
      ...
      <main>
        <NbSection ref={flowRef} ariaLabel="Architecture pipeline">
          ...
          <div className="nc-arch-stack nc-arch-part">
            ...
            <div className="nc-arch-stack-arrow" aria-hidden="true">\2193</div>
            <div className="nc-arch-frame">
          ...
          <div className="nc-arch-registers nc-arch-part">
            {SPECS.map((spec, i) => (
              <div key={i} className="nc-arch-register">
                ...
                <span className={`nc-arch-register-val ${spec.amber ? "nc-arch-register-val--amber" : ""}`}>
                  {spec.val}
                </span>
