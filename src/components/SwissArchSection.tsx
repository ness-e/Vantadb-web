...
const LAYERS = [
  { id: "app", name: "PYTHON APP", type: "CLIENT LAYER", color: "var(--foreground)", size: "small" },
  { id: "pyo3", name: "PYO3 BRIDGE", type: "FFI BOUNDARY", color: "var(--amber)", size: "medium" },
  { id: "rust", name: "RUST CORE", type: "ENGINE LAYER", color: "var(--foreground)", size: "large" },
  { id: "storage", name: "HNSW + WAL", type: "STORAGE LAYER", color: "var(--steel)", size: "large" },
];
        ...
        <div className="swiss-grid" style={{ gap: "24px" }}>
          ...
          <div style={{ gridColumn: "1 / 5" }}>
            ...
            <p style={{ fontFamily: "var(--font-sans)", fontSize: "var(--text-body)", color: "var(--muted)", lineHeight: 1.6 }}>
              Traditional vector databases require serialization, HTTP/gRPC transport, and context switching.
              <br /><br />
              VantaDB lives in the same memory space as your application. The PyO3 bridge provides native zero-copy access to the Rust core.
            </p>
          </div>
              ...
              {LAYERS.map((layer, index) => (
                <div 
                  ...
                  style={{
                    position: "relative",
                    width: "100%",
                    height: layer.size === "large" ? "140px" : layer.size === "medium" ? "100px" : "80px",
                  ...
                  onMouseEnter={(e) => {
                    ...
                    allLayers.forEach(l => { if (l !== e.currentTarget) (l as HTMLElement).style.opacity = "0.3"; });
                ...
                >
                  <div style={{ position: "absolute", top: "-10px", left: "24px", background: "var(--background)", padding: "0 8px" }}>
                    <span style={{ fontFamily: "var(--font-mono)", fontSize: "0.75rem", color: layer.color, fontWeight: 600, letterSpacing: "0.1em" }}>
                      {layer.type}
                    </span>
                  </div>
                  
                  <span style={{ fontFamily: "var(--font-display)", fontSize: "2rem", fontWeight: 700, letterSpacing: "-0.02em", color: "var(--foreground)" }}>
                    {layer.name}
                  </span>
                  ...
                  {index < LAYERS.length - 1 && (
                    <svg 
                      width="20" height="40" 
                      style={{ position: "absolute", bottom: "-32px", left: "50%", transform: "translateX(-50%)", zIndex: 0 }}
                    >
                      <line x1="10" y1="0" x2="10" y2="40" stroke="var(--border)" strokeWidth="1" strokeDasharray="4 4" />
                      <polygon points="5,35 15,35 10,40" fill="var(--border)" />
                    </svg>
                  )}
                  
                  {/* Dimensiones aproximadas */}
                  <div style={{ position: "absolute", right: "-40px", top: "50%", transform: "translateY(-50%)", display: "flex", alignItems: "center", gap: "8px" }}>
                    <div style={{ width: "16px", height: "1px", background: "var(--border)" }}></div>
                    <span style={{ fontFamily: "var(--font-mono)", fontSize: "0.7rem", color: "var(--steel)" }}>
